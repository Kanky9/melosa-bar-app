...
export default function AdminLayout({
  ...
  return (
    <div className="flex min-h-screen bg-muted/40">
      ...
      <header className="block md:hidden fixed top-0 left-0 right-0 h-16 bg-card border-b z-40 shadow-sm">
        <div className="flex items-center justify-between p-3 h-full"> {/* Changed p-4 to p-3 for potentially better alignment */}
          <Link href="/admin" className="text-xl font-bold text-primary" onClick={() => { setIsSheetOpen(false); }}>
             Melosa
          </Link>
          <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
            <SheetTrigger asChild>
              {/* Changed variant to primary for better visibility */}
              <Button variant="primary" size="icon" aria-label="Abrir menú">
